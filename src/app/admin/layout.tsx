...
export default async function AdminLayout({ children }: { children: React.ReactNode }) {
  const role = await requireRole(["admin", "manager"], "/admin/tournaments");
  return (
    <div className="flex min-h-screen flex-col">
      <header className="flex items-center justify-between border-b border-border px-4 py-3">
        <Link href="/admin/tournaments" className="flex items-center gap-2">
          <span className="text-sm font-bold uppercase tracking-widest text-accent">Move Beyond</span>
          <span className="text-sm font-semibold text-muted">Tournament Console</span>
        </Link>
